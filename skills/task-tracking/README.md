...
```
$KIRO_HOME/data/task-tracking/<id>.json
```

Override with `TASK_TRACKING_DIR=/absolute/path` — useful for
...
- Does not expire old data. `archive` hides a workbench from listings;
  pruning is manual (`rm $KIRO_HOME/data/task-tracking/<id>.json`).
