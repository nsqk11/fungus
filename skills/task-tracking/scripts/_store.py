"""Storage helpers for task-tracking.

Owns:

- Data directory resolution (``TASK_TRACKING_DIR`` env var, fallback
...
def data_dir() -> Path:
    """Return the directory that holds task-tracking JSON files.

    Resolution order:

    1. ``TASK_TRACKING_DIR`` environment variable (absolute path).
    2. Fallback: ``~/.kiro/data/task-tracking/``.
    """
    override = os.environ.get("TASK_TRACKING_DIR")
    if override:
        return Path(override)
    d = Path.home() / ".kiro" / "data" / "task-tracking"
